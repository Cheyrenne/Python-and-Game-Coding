...
class BJ_Game(object):
    ...
    def play(self):
        ...
        if len(self.deck.cards) < 52:              # if playing again and deck isn't full
            self.deck.populate() 
            self.deck.shuffle()
...
class War_Game(BJ_Game):
    ...
    def play(self):
        ...
        winner = []
        print(self.dealer)
        max = self.dealer.total
        for player in self.players:
            if player.total > max:
                max = player.total
                winner= []
                winner.append(player)
            if player.total == max and player not in winner:  # if there is a draw among players
                winner.append(player)
            print(player)
        if winner != []:
            if len(winner) > 1:
                print("It's a draw! Between", winner)
            else:
                print(winner[0], " wins!\n")          
        else:
